"""
CUDA/C++ kernel parser using regex-based analysis
"""
...
class CUDAParser(BaseParser):
    """Parser for CUDA and C++ kernel code"""
    
    def __init__(self):
        super().__init__(LanguageType.CUDA)
        
        # Regex patterns for CUDA/C++ parsing
        self.kernel_pattern = re.compile(r'__global__\s+.*?\s+(\w+)\s*\([^)]*\)\s*{', re.MULTILINE | re.DOTALL)
        self.function_pattern = re.compile(r'(?:__device__\s+)?(?:__host__\s+)?(?:__forceinline__\s+)?(\w+)\s+(\w+)\s*\([^)]*\)\s*{', re.MULTILINE)
        self.variable_pattern = re.compile(r'(?:__shared__\s+|__constant__\s+|__device__\s+)?(?:const\s+)?(\w+(?:\s*\*\s*)*)\s+(\w+)(?:\[([^\]]+)\])?', re.MULTILINE)
        self.loop_pattern = re.compile(r'(for|while)\s*\([^)]*\)\s*{', re.MULTILINE)
        self.sync_pattern = re.compile(r'(__syncthreads|__syncwarp|__threadfence|barrier)\s*\([^)]*\)', re.MULTILINE)
    ...
    def detect_language(self, code: str) -> bool:
        """Detect if this is CUDA/C++ code"""
        cuda_indicators = [
            '__global__', '__device__', '__host__', '__shared__',
            '__constant__', '__syncthreads', 'cudaMalloc', 'cudaMemcpy',
            'dim3', 'blockIdx', 'threadIdx', 'gridDim', 'blockDim'
        ]
        
        return any(indicator in code for indicator in cuda_indicators)
    
    async def parse(self, code: str) -> ParsedKernel:
        """Parse CUDA/C++ kernel code"""
        lines = code.split('\n')
    ...
    def _parse_functions(self, code: str) -> List[FunctionSignature]:
        ...
        for match in kernel_matches:
            ...
            functions.append(FunctionSignature(
                name=func_name,
                return_type="void",  # Most kernels return void
    ...
    def _parse_variables(self, code: str) -> List[VariableDeclaration]:
        ...
        for i, line in enumerate(lines):
            ...
            for match in var_matches:
                ...
                is_shared = '__shared__' in line
                is_global = '__global__' in line or '__device__' in line
                ...
                variables.append(VariableDeclaration(
                    ...
                    is_shared=is_shared,
                    is_global=is_global,
                    is_local=not (is_shared or is_global),
    ...
    def _parse_memory_accesses(self, code: str) -> List[MemoryAccess]:
        ...
        for i, line in enumerate(lines):
            ...
            for match in mem_matches:
                ...
                is_shared = '__shared__' in line or 'shared' in var_name.lower()
                is_global = '__global__' in line or 'global' in var_name.lower()
