...
__global__ void conv2d_kernel(const float* input, const float* weight, const float* bias, float* output,
    ...
    if (out_channel < out_channels && out_height < in_height - kernel_size + 1) {
        for (int out_width = 0; out_width < in_width - kernel_size + 1; ++out_width) {
            float sum = bias[out_channel];
            for (int k = 0; k < kernel_size; ++k) {
                for (int l = 0; l < kernel_size; ++l) {
                    for (int in_channel = 0; in_channel < in_channels; ++in_channel) {
                        int input_index = (in_channel * in_height + out_height + k) * in_width + out_width + l;
                        int weight_index = (out_channel * in_channels + in_channel) * kernel_size * kernel_size + k * kernel_size + l;
                        sum += input[input_index] * weight[weight_index];
                    }
                }
            }
            int output_index = (out_channel * in_height + out_height) * in_width + out_width;
