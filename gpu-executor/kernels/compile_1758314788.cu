...
__global__ void matrixMultiplyKernel(const float *A, const float *B, float *C, int widthA, int widthB, int heightA) {
    ...
    // Check if the thread is within the bounds of the resulting matrix C
    if (row < heightA && col < widthB) {
        float sum = 0.0f;
        // Perform matrix multiplication for this element of C
...
int main() {
    ...
    printf("Resulting matrix C (first 5x5 elements):\n");
    for (int i = 0; i < 5; ++i) {
        for (int j = 0; j < 5; ++j) {
